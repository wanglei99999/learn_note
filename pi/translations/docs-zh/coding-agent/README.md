...
> 新贡献者提交的 issue 和 PR 默认会被自动关闭。维护者每天会审阅被自动关闭的 issue。参见 [CONTRIBUTING.md](https://github.com/earendil-works/pi/blob/main/CONTRIBUTING.md)。
...
对每个内置 provider，pi 维护一份支持工具调用的模型列表。已配置 provider 的模型目录会自动刷新；运行 `pi update --models` 可强制立即刷新。通过订阅（`/login`）或 API key 认证后，即可通过 `/model`（或 Ctrl+L）选择该 provider 的任意模型。
...
<p align="center"><img src="https://raw.githubusercontent.com/earendil-works/pi/main/packages/coding-agent/docs/images/interactive-mode.png" alt="Interactive Mode" width="600"></p>
...
- **Alt+Enter** 将消息作为 *follow-up* 消息（后续消息）入队，仅在 agent 完成所有工作后投递
...
长会话可能耗尽上下文窗口。Compaction（上下文压缩）会总结较早的消息，同时保留较近的消息。
...
用 `.pi/SYSTEM.md`（项目）或 `~/.pi/agent/SYSTEM.md`（全局）替换默认的系统提示词。通过 `APPEND_SYSTEM.md` 追加而不替换。
...
以 Markdown 文件形式存在的可复用 prompt（提示词模板）。输入 `/name` 展开。
...
<p align="center"><img src="https://raw.githubusercontent.com/earendil-works/pi/main/packages/coding-agent/docs/images/doom-extension.png" alt="Doom Extension" width="600"></p>
...
参见 [sdk.md](./sdk.md) 和 [examples/sdk/](https://github.com/earendil-works/pi/tree/main/packages/coding-agent/examples/sdk/)。
...
指南参见 [CONTRIBUTING.md](https://github.com/earendil-works/pi/blob/main/CONTRIBUTING.md)，设置、fork 和调试参见 [development.md](./development.md)。
...
<p align="center">
  ...
  <br /><br />
  <a href="https://exe.dev"><img src="https://raw.githubusercontent.com/earendil-works/pi/main/packages/coding-agent/docs/images/exy.png" alt="Exy mascot" width="48" /><br />exe.dev</a>
</p>

...
>
> 所有 `文件:行号` 基于 commit `859bd29bd`。核心文件四个：`core/agent-session.ts`（结算与覆盖）、`core/system-prompt.ts`（拼装）、`core/extensions/runner.ts`（链式改写）、`packages/agent/src/agent-loop.ts`（每轮取用）。
...
```typescript
// agent-loop.ts:451  出去之后，模型点名调工具时按名字查表
const hasSequentialToolCall = toolCalls.some(
	(tc) => currentContext.tools?.find((t) => t.name === tc.name)?.executionMode === "sequential",
);
// agent-loop.ts:638
const tool = currentContext.tools?.find((t) => t.name === toolCall.name);
if (!tool) { /* 返回 immediate 错误 */ }
```

```text
发出去时  →  convertTools 序列化成 JSON schema，告诉模型"你可以调这些"
回来时    →  按名字查表，找到 execute 真去执行
```

**同一个数组，出去时是说明书，回来时是调度表。** 10 篇讲的 T3 段工具执行，查的就是它。
...
**可替换的是"人格与指令"，不可绕过的是"这个会话的客观事实"。** 即便你完全自定义系统提示词，`AGENTS.md`、skill 清单、cwd 还是会被追加上去。
...
原生 function calling 不是"模型凭空知道"——API 服务端会把 `tools` 数组**渲染进模型的 token 流**，用模型训练过的固定格式。`convertTools` 里留了三处佐证：

- **`cache_control` 只打在最后一个工具上**：提示词缓存按前缀匹配，"在最后一个工具后切一刀"只有当工具是连续有序的 token 前缀时才有意义。
- **存在 `defer_loading`**：说明工具定义有 token 成本，多到一定程度需要按需加载。
- **工具计入 input tokens**：加十个工具，输入 token 会涨。

**所以"工具不在系统提示词里"的准确含义是：不在开发者手写的那段文本里**，而不是模型看不见。它比手写 `toolname: description` 可靠，不是机制上更高级，而是**格式约定被固化进了权重里**。

### 4.4 由此看 pi 那份清单：一半冗余，一半不可替代
...
### 4.5 对照：skill 为什么必须走提示词（`core/skills.ts:362`）
...
`Object.defineProperties({}, Object.getOwnPropertyDescriptors(...))` 拷的是**属性描述符**——`createContext()` 返回的对象里有懒 getter，直接展开会当场求值。拷描述符保住惰性，同时允许在副本上覆盖一个方法而**不污染原对象**：别的事件里 `getSystemPrompt()` 该读会话字段还是读会话字段。
...
12. **可替换主体，不可绕过事实。** `customPrompt` 换掉人格与指令，但项目上下文、skill 清单、cwd 照常追加。
13. **不伪造信息，宁可缺席。** 没有 `promptSnippet` 就不进清单，而不是拿 description 截断顶替；配一句兜底说明覆盖缺口。
14. **依赖工具集组合的指导只能在拼装时算。** 单个工具的 description 不可能知道别的工具在不在场。
15. **按读取时机决定详略。** 每轮通读的位置放一行，选定后才读的位置放长文。
16. **能力不具备就不暴露选项。** 没有 read 工具就不列 skill 清单。
17. **保留归属。** 每个 `AGENTS.md` 一个带 `path` 的 XML 节点，不拼成一坨——与 `sourceInfo` 同一模式。
18. **分层存储、取用时合成。** 两个正交的变化源各占一个字段，`override ?? base` 求值时才合并；提前压平就再也拆不开。
19. **"没人改"必须等价于"改回原样"。** 显式 `else` 复位 + `finally` 清空，双保险。
20. **同一信息的多个读取入口必须一致。** 重写 `ctx.getSystemPrompt` 消除 `event.systemPrompt` 与 `ctx` 之间的分歧，否则单扩展测试正常、多扩展并存出错。
21. **拷贝属性描述符以保住惰性。** `Object.getOwnPropertyDescriptors` 而非展开，既保留懒 getter 又能在副本上局部覆盖而不污染原对象。
22. **把原料一并交给扩展。** 不只给拼好的字符串，还给 `BuildSystemPromptOptions`，让扩展重拼而不是正则替换成品。
23. **快照用 `.slice()`。** 当轮固定的清单必须是拷贝，执行途中源数据变化不得影响已发出的内容。
...
11. `Context.tools` 的"双重身份"指什么？
12. `_rebuildSystemPrompt` 凑齐了哪几类料？为什么要把 `_baseSystemPromptOptions` 存下来？
13. `customPrompt` 替换掉了什么、没替换掉什么？这个边界的道理是什么？
14. 没写 `promptSnippet` 的扩展工具会怎样？它还能被模型调用吗？
15. `description`、`promptSnippet`、`promptGuidelines` 三者的分工是什么？判据是什么？
16. 那条 bash guideline 为什么只在没有 grep/find/ls 时才加？它为什么不能写进 bash 的 description？
17. 原生 function calling 下模型怎么知道有哪些工具？`convertTools` 里哪三处能佐证工具进了 token 流？
18. pi 的 `Available tools` 清单是冗余的吗？`Guidelines` 呢？判据是什么？
19. skill 为什么必须写进系统提示词？`<location>` 为什么不可缺？
20. `disableModelInvocation` 是怎么实现的？为什么同样的手法对工具无效？
21. `_systemPromptOverride` 为什么要单独成字段？三条理由里哪条是决定性的？
22. 「链式」和「广播」「收集」「短路投票」的区别是什么？没有链式会怎样？
23. `ctx.getSystemPrompt` 为什么要在 `before_agent_start` 里被重写？不重写会出现什么样的 bug？
24. `Object.getOwnPropertyDescriptors` 而不是展开运算符，解决了什么问题？
25. `else` 分支的复位和 `finally` 的清空，各防什么？
26. `prepareNextTurnWithContext` 里的 `.slice()` 是为了什么？
27. 完整复述：一个扩展工具从磁盘文件到被模型调用，要经过哪些环节？

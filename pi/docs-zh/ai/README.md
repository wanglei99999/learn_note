...
你可以在不发请求的情况下检查解析结果。传 provider ID 获取 provider 作用域的认证，或传一个模型以包含其静态的 `model.headers`：
...
if (modelAuth) {
  console.log(`configured via ${modelAuth.source}`); // 例如 "ANTHROPIC_API_KEY"、"OAuth"、"stored credential"
  console.log(modelAuth.auth.headers);              // Provider 认证 headers + model.headers
} else {
...
Header 名称按不区分大小写的方式合并。显式 headers 覆盖认证/模型 headers，而 transform 拥有最终控制权；对某个 header 返回 `null` 会抑制支持删除的更低层默认值。

`transformHeaders` 属于 `Models`，而不是 `Provider`。`Models` 实现必须消费它，并在调用 `Provider.stream*()` 之前把它移除。Provider 实现继续接收普通的 `ApiStreamOptions` 或 `SimpleStreamOptions`，绝不自行处理该 transform。请使用这个选项，而不要在 `stream*()` 之前调用 `getAuth(model)`——那会把请求认证解析两次。
...
`xhigh` 和 `max` 是模型特定的可选（opt-in）级别。使用 `getSupportedThinkingLevels(model)` 判断具体模型是否暴露其中某个级别；像 GPT-5.6 这样的模型可以同时暴露两者。
...
Provider 级的端点或请求转换应放在 provider 的 API 实现中：包装你作为 `api` 传入的 `ProviderStreams`，让每个请求在分发前都先经过该转换。Cloudflare 的 provider 就用这种方式，从已解析的 provider env 中物化 account/gateway 端点占位符：
...
使用模型级的 `thinkingLevelMap` 来描述模型特定的 thinking 控制。key 是 pi 的 thinking 级别（`off`、`minimal`、`low`、`medium`、`high`、`xhigh`、`max`）。`high` 及以下的标准级别缺失时使用 provider 默认值；`xhigh` 和 `max` 是可选（opt-in）级别，需要非 null 的 map 条目。字符串值会发送给 provider，`null` 表示不支持该级别，map 可以跳过某些级别。
